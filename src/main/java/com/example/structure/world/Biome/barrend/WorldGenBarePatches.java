...
public class WorldGenBarePatches extends WorldGenerator {
    ...
    public void intereperetCircle(double x, double y, double z, World world, int radius) {
        ...
        for(BlockPos blockPos : affectedConversionPositions) {
            ...
            for(int c = (int) y + 9; c >= y - 10; c--) {
                ...
                if(world.getBlockState(findSmooth) == Blocks.END_STONE.getDefaultState() && world.getBlockState(findSmooth.up()) != Blocks.CHORUS_PLANT.getDefaultState()) {
                    world.setBlockState(new BlockPos(blockPos.getX(), c, blockPos.getZ()), ModBlocks.BARE_SANS.getDefaultState());
