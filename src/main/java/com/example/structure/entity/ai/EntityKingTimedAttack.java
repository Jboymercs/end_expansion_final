...
public class EntityKingTimedAttack<T extends EntityAbstractEndKing & IAttack> extends EntityAIBase {
    ...
    public EntityKingTimedAttack(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityKingTimedAttack(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if(!entity.lockLook) {
        if (distSq <= this.maxAttackDistSq && canSee && !this.entity.IPhaseTwo && !this.entity.isMeleeMode) {
            this.entity.getNavigator().clearPath();
            ++this.strafingTime;
        } else {
            if (this.entity.IPhaseTwo || this.entity.isMeleeMode) {
