...
public class EntityAiSeekerPrime <T extends EndSeekerPrime & IAttack> extends EntityAIBase {
    ...
    public EntityAiSeekerPrime(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount) {
        this(entity, moveSpeedAmp, attackCooldown, maxAttackDistance, strafeAmount, 30.0f);
    }

    public EntityAiSeekerPrime(T entity, double moveSpeedAmp, int attackCooldown, float maxAttackDistance, float strafeAmount, float lookSpeed) {
        this.entity = entity;
    ...
    public void move(EntityLivingBase target, double distSq, boolean canSee) {
        if (!entity.lockLook) {
            if (distSq <= this.maxAttackDistSq && canSee && !entity.isMeleeMode) {
