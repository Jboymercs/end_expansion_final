package com.jboymercs.endexpansion.entity.trader;

import com.jboymercs.endexpansion.config.ModConfig;
import com.jboymercs.endexpansion.entity.ai.EntityAIAvalon;
import com.jboymercs.endexpansion.entity.trader.action.ActionFarRange;
import com.jboymercs.endexpansion.entity.trader.action.ActionLineAOE;
import com.jboymercs.endexpansion.entity.trader.action.ActionMediumRangeAOE;
import com.jboymercs.endexpansion.entity.trader.action.ActionShortRangeAOE;
import com.jboymercs.endexpansion.entity.util.IAttack;
import com.jboymercs.endexpansion.init.ModItems;
import com.jboymercs.endexpansion.util.*;
...
import com.jboymercs.endexpansion.world.api.trader.WorldGenTraderArena;
...
public class EntityAvalon extends EntityAbstractAvalon implements IAnimatable, IAnimationTickable, IAttack {
    ...
    private EntityPlayer getNearbyPlayer(double range) {
        return getNearbyPlayer(range, false);
    }

    private EntityPlayer getNearbyPlayer(double range, boolean requireSurvivalPlayer) {
        AxisAlignedBB searchBox = this.getEntityBoundingBox().grow(range);
        for(EntityPlayer player : this.world.playerEntities) {
            if(!player.getIsInvulnerable() && player.getEntityBoundingBox().intersects(searchBox)) {
                if(!requireSurvivalPlayer || (!player.isSpectator() && !player.isCreative())) {
                    return player;
                }
            }
        }
    ...
    public void onUpdate() {
       ...
       EntityPlayer nearbyBossPlayer = this.isIAmBoss() || IAmAggroed ? getNearbyPlayer(30D) : null;
       if(this.isIAmBoss() && !IAmAggroed && nearbyBossPlayer != null) {
        ...
        if(target == null && !this.isIAmBoss()) {
            ...
            EntityPlayer nearbyPlayer = getNearbyPlayer(5D, true);

            if (nearbyPlayer != null && !this.isOpenState() && !this.transitionTooOpen) {
                stateTransitionTooOpen();
            }

            if (nearbyPlayer == null && !this.isSummonState() && !this.isDeathState()) {
            ...
            if (!this.IAmAggroed) {
                List<EntityItem> nearbyCoins = this.world.getEntitiesWithinAABB(EntityItem.class, this.getEntityBoundingBox().grow(5D),
                        e -> !e.getIsInvulnerable() && e.getItem().getItem() == ModItems.OBSIDIAN_COIN);

                if (!nearbyCoins.isEmpty()) {
                    EntityItem item = nearbyCoins.get(0);
                    item.setDead();
