...
import com.jboymercs.endexpansion.init.ModProfressions;
...
public class EntityAbstractAvalon extends EntityTrader implements IEntityMultiPart, IPitch {
  ...
  if(this.isIAmBoss() && !world.isRemote) {
      AxisAlignedBB healthBlockBox = getEntityBoundingBox().grow(20, 8, 20);
      if (currentHealth < 0.75 && !hasLaunchedBlocksOne) {
          for (int i = 0; i < 4; i++) {
              BlockPos setTooPos = ModUtils.avalonSearchForBlocks(healthBlockBox, world, this, ModBlocks.EYED_OBSIDIEAN.getDefaultState());
              if (setTooPos != null) {
      ...
      if(healthBlockCheckTimer-- <= 0) {
          healthBlockCheckTimer = 20;
          BlockPos setTooPos = ModUtils.searchForBlocks(healthBlockBox, world, ModBlocks.OBSIDIAN_HEALTH_BLOCK.getDefaultState());
          if (setTooPos == null) {
              if (hasLaunchedBlocksThree) {
