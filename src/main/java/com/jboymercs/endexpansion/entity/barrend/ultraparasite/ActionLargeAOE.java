package com.jboymercs.endexpansion.entity.barrend.ultraparasite;
...
public class ActionLargeAOE implements IAction {

    private boolean secondpart;

    public ActionLargeAOE(boolean secondPart) {
    ...
    public void performAction(EntityModBase actor, EntityLivingBase target) {
        ...
        if(this.secondpart) {
            ...
            ModUtils.circleCallback(14, 78, (pos)-> {
                pos = new Vec3d(pos.x, 0, pos.y).add(actor.getPositionVector());
                int y = getSurfaceHeight(actor.world, new BlockPos(pos.x, 0, pos.z), (int) actor.posY - 5, (int) actor.posY + 3);
                EntityParasiteBombAOE spike = new EntityParasiteBombAOE(actor.world);
                spike.setPosition(pos.x, y + 1, pos.z);
