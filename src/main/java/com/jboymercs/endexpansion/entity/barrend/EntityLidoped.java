...
import com.jboymercs.endexpansion.entity.ai.EntityAILidopedHome;
...
public class EntityLidoped extends EntityBarrendMob implements IAttack, IAnimatable, IAnimationTickable {
    ...
    public void onUpdate() {
        ...
        if(target == null) {
            ...
            if(this.isJumping) {
                if(!target.getIsInvulnerable() && target.getEntityBoundingBox().intersects(this.getEntityBoundingBox().grow(1.5D))) {
                    if(target instanceof EntityLidoped) {
                        if(!((EntityLidoped) target).isAttachedHead()) {
                            //attach Too head
