...
public abstract class RenderGeoExendedLidoped<T extends EntityModBase & IAnimatable> extends GeoEntityRenderer<T> {
    ...
    protected RenderGeoExendedLidoped(RenderManager renderManager, AnimatedGeoModel<T> modelProvider) {
        this(renderManager, modelProvider, 1D, 1D, 0);
    }

    protected RenderGeoExendedLidoped(RenderManager renderManager, AnimatedGeoModel<T> modelProvider, double widthScale, double heightScale, float shadowSize) {
    ...
    public void renderRecursively(BufferBuilder builder, GeoBone bone, float red, float green, float blue, float alpha) {
        ...
        if (this.currentModelRenderCycle == 0) {
            ...
            if (boneItem != null || boneBlock != null) {
                ...
                GlStateManager.translate(0.0f, 0.30f, 0.05f);
