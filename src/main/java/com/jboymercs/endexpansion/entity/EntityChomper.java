...
public class EntityChomper extends EntityModBase implements IAnimationTickable, IAnimatable, IAttack {
    ...
    public void onUpdate() {
        ...
        if(this.isCurrentlyDugIn && target == null) {
            ...
            warnCooldown--;
            AxisAlignedBB searchBox = this.getEntityBoundingBox().grow(8D);
            AxisAlignedBB closeBox = this.getEntityBoundingBox().grow(4D);
            List<EntityLivingBase> nearbyEntities = this.world.getEntitiesWithinAABB(EntityLivingBase.class, searchBox, e -> !e.getIsInvulnerable());
            ...
            for(EntityLivingBase base : nearbyEntities) {
                ...
                if(base instanceof EntityPlayer) {
                    ...
                    canReact = !player.isSneaking() && !player.isCreative();
                }
                ...
                if(!this.hasStartedPopOut && base.getEntityBoundingBox().intersects(closeBox) && this.getEntitySenses().canSee(base)) {
                    this.statePopOut(base);
                    this.hasStartedPopOut = true;
                }
            }
