...
import com.jboymercs.endexpansion.entity.ai.snatcher.EntityStalkAI;
   ...
   addEvent(()-> {
    ...
    public void onUpdate() {
        ...
        if(this.isCurrentlyinHibernation) {
            ...
            AxisAlignedBB closeBox = this.getEntityBoundingBox().grow(MobConfig.stalker_distance);
            List<EntityPlayer> farTargets = this.world.getEntitiesWithinAABB(EntityPlayer.class, this.getEntityBoundingBox().grow(30D), e -> !e.getIsInvulnerable());
            if(!farTargets.isEmpty()) {
                hibernationTimer--;
            }

            if(!this.setOnaggro) {
                for(EntityPlayer player : farTargets) {
                    if(player.getEntityBoundingBox().intersects(closeBox)) {
                        this.endHiberntaion();
