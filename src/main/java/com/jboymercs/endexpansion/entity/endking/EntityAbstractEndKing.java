package com.jboymercs.endexpansion.entity.endking;

import com.jboymercs.endexpansion.config.MobConfig;
import com.jboymercs.endexpansion.config.ModConfig;
import com.jboymercs.endexpansion.entity.EntityEye;
import com.jboymercs.endexpansion.entity.EntityModBase;
import com.jboymercs.endexpansion.entity.ai.IMultiAction;
import com.jboymercs.endexpansion.entity.endking.EndKingAction.ActionShootLazer;
...
import com.jboymercs.endexpansion.entity.util.IPitch;
import com.jboymercs.endexpansion.renderer.ITarget;
...
public class EntityAbstractEndKing extends EntityModBase implements IEntityMultiPart, IPitch, DirectionalRender, ITarget {
    ...
    private int nearbyAttackBlockerCheckTimer = 0;
    ...
    public void onUpdate() {
        ...
        if(!this.IPhaseThree) {
            ...
            int posY = MathHelper.floor(this.posY);
            int posZ = MathHelper.floor(this.posZ);
            BlockPos.MutableBlockPos checkPos = new BlockPos.MutableBlockPos();
            for (int i = 0; i > -10; i--) {
                if (!world.isAirBlock(checkPos.setPos(posX, posY + i, posZ))) {
                    hasGround = true;
                    break;
                }
            }
        ...
        if(!world.isRemote && nearbyAttackBlockerCheckTimer-- <= 0) {
            nearbyAttackBlockerCheckTimer = 5;
            ...
            this.hasEyesNearby = !nearbyEyes.isEmpty();
        }
        ...
        if(!world.isRemote && nearbyAttackBlockerCheckTimer == 5) {
            ...
            hasSwordsNearby = !nearbySwords.isEmpty();
        }
    ...
    public void onLivingUpdate() {
        ...
        if(this.isBossStall()) {
            ...
            if(!hasStartedBossFight && this.world.isAnyPlayerWithinRangeAt(this.posX, this.posY, this.posZ, 8D)) {
                startBossFight();
            }
    ...
    public void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue((double) MobConfig.end_king_health * getHealthModifierAsh());
        ...
        this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(MobConfig.end_king_damage * getAttackModifierAsh());
    }
