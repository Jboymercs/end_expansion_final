    ...
    public void onLivingUpdate() {
        ...
        if(this.iAmBossMob && target != null) {
            if(!this.hasStartedScaling && target instanceof EntityPlayer && !this.world.isRemote) {
                ServerScaleUtil.ScaledStats scaledStats = ServerScaleUtil.getScaledStats(this, world);
                //This is change the Health in accordance with how many players are currently nearby // TEST
                this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(scaledStats.maxHealth);
                this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(scaledStats.attackDamage);
