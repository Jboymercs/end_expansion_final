package com.jboymercs.endexpansion.event_handler.client;
...
public class RenderPotionInArena {

    private static final ResourceLocation[] TEXTURES = new ResourceLocation[10];

    private static final float ANIMATION_SPEED = 0.004f;
    private static final float FADE_DISTANCE_SQUARED = 9;

    static {
        for(int i = 0; i< TEXTURES.length; i++){
            TEXTURES[i] = new ResourceLocation(ModReference.MOD_ID, "textures/environment/barrier/barrier_" + i + ".png");
        }
    }
    ...
    public static void onRenderWorldLastEvent(RenderWorldLastEvent event){
        ...
        if(player.isPotionActive(ModPotions.IN_ARENA)){

            Vec3d centre = ModUtils.getCentre(NBTUtil.getPosFromTag(player.getEntityData().getCompoundTag(PotionInArena.ENTITY_TAG)));
            float r = PotionInArena.getArenaDistance(player.getActivePotionEffect(ModPotions.IN_ARENA).getAmplifier());
            ...
            float alpha = Math.min(1, player.getActivePotionEffect(ModPotions.IN_ARENA).getDuration() / 40f);
