...
import com.jboymercs.endexpansion.entity.EntitySnatcher;
...
import com.jboymercs.endexpansion.world.Biome.decorator.EEBiomeDecorator;
import com.jboymercs.endexpansion.world.Biome.generation.*;
...
import com.jboymercs.endexpansion.world.islands.WorldGenOutpost;
...
public class BiomeAshWasteland extends BiomeFogged implements IEndBiome, INetherAPIRegistryListener, IAmbienceBiome {
    ...
    public void populate(@Nonnull INetherAPIChunkGenerator chunkGenerator, int chunkX, int chunkZ) {
        ...
        //Outpost
        if(canStructureSpawn(pos.getX(), pos.getZ(), world, 30)) {
            this.canGenerateKnightFortress(world, pos);
        }

        //Ashed Towers
        int towerGroundHeight = getGroundFromAbove(world, pos.getX(), pos.getZ());
        if(towerGroundHeight > 58) {
            ash_tower.generate(world, random, pos);
        }

        //Ashed Mines
        int minesGroundHeight = getGroundFromAbove(world, pos.getX(), pos.getZ());
        if(minesGroundHeight > 60) {
            ashed_mines.generate(world, random, pos);
        }
