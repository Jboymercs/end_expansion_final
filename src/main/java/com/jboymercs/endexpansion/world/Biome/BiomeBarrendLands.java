...
import com.jboymercs.endexpansion.entity.barrend.EntityLidoped;
...
import com.jboymercs.endexpansion.world.Biome.barrend.*;
import com.jboymercs.endexpansion.world.Biome.barrend.portal.WorldGenBarePortal;
import com.jboymercs.endexpansion.world.Biome.barrend.spires.WorldGenSpireBase;
import com.jboymercs.endexpansion.world.Biome.ore.WorldGenStarShard;
...
public class BiomeBarrendLands extends BiomeFogged implements IEndBiome, INetherAPIRegistryListener, IAmbienceBiome {
    ...
    public WorldGenerator barrendHeights = new WorldGenSpireBase();
    ...
    private final WorldGenerator barrend_portal = new WorldGenBarePortal("bare_portal");
    ...
    public void decorate(World world, Random rand, BlockPos pos) {
        ...
        if (world.rand.nextInt(4) == 0) {
            for (int k2 = 0; k2 < ModRand.range(1, 4); k2++) {
                int l6 = random.nextInt(12) + 2;
                int k10 = random.nextInt(12) + 2;
                int depthSignature = 2;
                for (int y = 160; y > 32; y--) {
                    IBlockState currentBlock = world.getBlockState(pos.add(l6, y, k10));
                    if (depthSignature == 1) {
        ...
        //Sand Patches
        for(int k2 = 0; k2 < ModRand.range(1, 3);k2++) {
            int l6 = random.nextInt(8) + 4;
            int k10 = random.nextInt(8) + 4;
            int depthSignature = 2;
            for(int y = 160; y > 32; y--) {
                IBlockState currentBlock = world.getBlockState(pos.add(l6, y, k10));
                if(depthSignature == 1) {
    ...
    public void populate(@Nonnull INetherAPIChunkGenerator chunkGenerator, int chunkX, int chunkZ) {
        ...
        //Barrend Arena
        int arenaGroundHeight = getGroundFromAbove(world, pos.getX(), pos.getZ());
        if(arenaGroundHeight > 58) {
            barrend_arena.generate(world, random, pos.add(0, arenaGroundHeight -2, 0));
        }

        //Archways and Camps
        int archGroundHeight = getGroundFromAbove(world, pos.getX(), pos.getZ());
        if(archGroundHeight > 55) {
            WorldGenBarrendArches archTooGenerate = ModRand.choice(arches);
            archTooGenerate.generate(world, random, pos.add(0, archGroundHeight -4, 0));
        }

        //Barrend Portal Summoning Structure
        int portalGroundHeight = getGroundFromAbove(world, pos.getX(), pos.getZ());
        if(portalGroundHeight > 100) {
            barrend_portal.generate(world, random, pos.add(0, portalGroundHeight - 2, 0));
        }

        //Barrend Crypts
        int cryptGroundHeight = getGroundFromAbove(world, pos.getX(), pos.getZ());
        if(cryptGroundHeight > 58 && ModConfig.dev_stuff_enabled) {
            if(cryptGroundHeight < 61) {
                barrend_crypt.generate(world, random, pos.add(0, cryptGroundHeight, 0));
            }
        }

        //StarShard Ore
       // genOre(ModBlocks.STAR_SHARD_ORE.getDefaultState(), ModRand.range(1, 4), 10, 80, 160, chunkGenerator.getWorld(), random, chunkX, chunkZ);
    ...
    public static int getGroundFromAbove(World world, int x, int z)
    {
        ...
        {
            Block blockAt = world.getBlockState(checkPos.setPos(x, y, z)).getBlock();
    ...
    private int getEndSurfaceHeight(World world, BlockPos pos, int min, int max)
    {
        ...
        while(currentY >= minY)
        {
            if(!world.isAirBlock(checkPos.setPos(pos.getX(), pos.getY() + currentY, pos.getZ())))
                return currentY;
            currentY--;
        }
        return 0;
