...
public class ModUtils {
    ...
    public static boolean anyBlocksMatch(World world, Vec3d origin, int xMin, int yMin, int zMin, int xMax, int yMax, int zMax, Predicate<BlockPos> predicate) {
        BlockPos.MutableBlockPos blockPos = new BlockPos.MutableBlockPos();
        int baseX = MathHelper.floor(origin.x);
        int baseY = MathHelper.floor(origin.y);
        int baseZ = MathHelper.floor(origin.z);
        for(int x = xMin; x < xMax; x++) {
            for(int y = yMin; y < yMax; y++) {
                for(int z = zMin; z < zMax; z++) {
                    blockPos.setPos(baseX + x, baseY + y, baseZ + z);
    ...
    public static void handleAreaImpact(float radius, Function<Entity, Float> maxDamage, Entity source, Vec3d pos, DamageSource damageSource,
        ...
        List<Entity> list = source.world.getEntitiesWithinAABBExcludingEntity(source, new AxisAlignedBB(posX, posY, posZ, posX, posY, posZ).grow(radius));

        for(Entity entity : list) {
            if(!(entity instanceof EntityLivingBase || entity instanceof MultiPartEntityPart || entity.canBeCollidedWith())) {
                continue;
            }

            AxisAlignedBB entityBox = entity.getEntityBoundingBox();
            double avgEntitySize = entityBox.getAverageEdgeLength() * 0.75;
            double centerX = entityBox.minX + (entityBox.maxX - entityBox.minX) * 0.5D;
            double centerY = entityBox.minY + (entityBox.maxY - entityBox.minY) * 0.5D;
            double centerZ = entityBox.minZ + (entityBox.maxZ - entityBox.minZ) * 0.5D;
            double centerDistance = Math.sqrt(squareDistance(centerX, centerY, centerZ, posX, posY, posZ));
            double headDistance = Math.sqrt(squareDistance(entity.posX, entity.posY + entity.getEyeHeight(), entity.posZ, posX, posY, posZ));
            double feetDistance = Math.sqrt(squareDistance(entity.posX, entity.posY, entity.posZ, posX, posY, posZ));
            double distance = Math.min(Math.min(centerDistance, headDistance), feetDistance);
            double adjustedDistance = Math.max(distance - avgEntitySize, 0);
            double adjustedDistanceSq = adjustedDistance * adjustedDistance;
            double damageFactor = damageDecay ? Math.max(0, Math.min(1, (radiusSq - adjustedDistanceSq) / radiusSq)) : 1;
            double damageFactorSq = damageFactor * damageFactor;
            ...
            if (damage > 0 && adjustedDistanceSq < radiusSq) {
                ...
                if(entity.attackEntityFrom(damageSource, (float) damage)) {
                    ...
                    double entitySizeFactorSq = entitySizeFactor * entitySizeFactor;
                    double velocityX = centerX - posX;
                    double velocityY = centerY - posY;
                    double velocityZ = centerZ - posZ;
                    double velocityLength = Math.sqrt(velocityX * velocityX + velocityY * velocityY + velocityZ * velocityZ);
                    if(velocityLength > 1.0E-4D) {
                        double velocityScale = damageFactorSq * knockbackFactor * entitySizeFactorSq / velocityLength;
                        entity.addVelocity(velocityX * velocityScale, velocityY * velocityScale, velocityZ * velocityScale);
                    }
                }
            }
        }
    ...
    public static BlockPos avalonSearchForBlocks(AxisAlignedBB box, World world, Entity entity, IBlockState block) {
        ...
        IBlockState healthBlock = ModBlocks.OBSIDIAN_HEALTH_BLOCK.getDefaultState();
        for (int x = i; x <= l; ++x) {
            for (int y = j; y <= i1; ++y) {
                for (int z = k; z <= j1; ++z) {
                    blockpos.setPos(x, y, z);
                    ...
                    if(iblockstate == block && world.getBlockState(abovePos.setPos(x, y + 1, z)) != healthBlock) {
        ...
        for (int dx = -radius_int; dx < radius_int + 1; dx++) {
            ...
            for (int dy = -y_lim; dy < y_lim + 1; dy++) {
                ...
                for (int dz = -z_lim; dz < z_lim + 1; dz++) {
                    ...
                    if ((power>1) ||(power > world.rand.nextDouble())){
                ...
                for (int z = k; z <= j1; ++z) {
                    ...
                    if (!block.isAir(iblockstate, world, blockpos) && iblockstate.getMaterial() != Material.FIRE) {
                        if (block != Blocks.COMMAND_BLOCK &&
                            ...
                            if (world.isAnyPlayerWithinRangeAt(blockpos.getX(), blockpos.getY(), blockpos.getZ(), 20)) {
    ...
    public static void leapTowards(EntityLivingBase entity, Vec3d target, float horzVel, float yVel) {
        ...
        double horzMag = Math.sqrt(entity.motionX * entity.motionX + entity.motionZ * entity.motionZ);
    ...
    public static int getSurfaceHeight(World world, BlockPos pos, int min, int max)
    {
        ...
        while(currentY >= min)
        {
            mutablePos.setPos(pos.getX(), currentY, pos.getZ());
            if(!world.isRemote && !world.isAirBlock(mutablePos) && world.getBlockState(mutablePos).isFullBlock()) {
                return currentY;
            }
    ...
    public static List<EntityLivingBase> getEntitiesInBox(Entity entity, AxisAlignedBB bb) {
        ...
        List<EntityLivingBase> entities = new ArrayList<>(list.size());
        for(Entity nearby : list) {
            if(nearby instanceof EntityLivingBase) {
                entities.add((EntityLivingBase) nearby);
            }
        }
        return entities;
    }
